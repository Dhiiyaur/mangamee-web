...
const SERVER_BASE_URL_MANGA_HEROKU = `https://mangamee-api.herokuapp.com/manga`
const SERVER_BASE_URL_MANGA = `http://108.136.161.127:1234/manga`
...
const fetchMeta = async(params) => {
    let url = `${SERVER_BASE_URL_MANGA}/meta/${params.source}/${params.mangaId}`
    return await fetch(url)
}

const fetchSearch = async(params) => {
    let url = `${SERVER_BASE_URL_MANGA}/search/${params.source}?title=${params.search}`
    return await fetch(url)
}
